...
import { publishTemplateComponent } from "@/modules/configuration/application/template-service";
...
async function publish(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return await idempotentCommandResponse({
      ...
      operation: "configuration.template-component.publish",
      idempotencyKey,
      request: { path, body },
      execute: async (transaction) => ({
        status: 201,
        body: await publishTemplateComponent(
          {
            ...body,
            code: path.code,
            actorId: guard.actor.id,
...
export const POST = withRequestObservability(
  { module: "configuration-templates", operation: "publish-component" },
  publish
);
