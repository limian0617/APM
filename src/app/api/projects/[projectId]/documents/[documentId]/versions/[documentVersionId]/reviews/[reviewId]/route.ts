...
import { decideDocumentReview } from "@/modules/documents/application/controlled-document-service";
...
async function decideReview(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const body = await parseJsonBody(request, decideDocumentReviewBodySchema);
    ...
    return await idempotentCommandResponse({
      actorId: guard.actor.id,
      operation: "projects.controlled-document.decide-review",
      idempotencyKey,
      request: { path, body },
      execute: async (transaction) => ({
        status: 200,
        body: await decideDocumentReview(
...
export const POST = withRequestObservability(
  { module: "documents", operation: "decide-controlled-document-review" },
  decideReview
);
