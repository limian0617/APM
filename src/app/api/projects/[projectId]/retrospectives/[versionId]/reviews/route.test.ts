...
const service = vi.hoisted(() => ({ reviewRetrospectiveVersion: vi.fn() }));
...
describe("retrospective review route", () => {
  it("denies before invoking review service", async () => {
    ...
    expect(service.reviewRetrospectiveVersion).not.toHaveBeenCalled();
  });
});
