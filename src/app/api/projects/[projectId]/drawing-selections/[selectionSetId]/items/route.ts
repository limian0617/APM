...
import { addDrawingSelectionItem } from "@/modules/drawings/application/drawing-selection-service";
...
async function addSelectionItem(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const body = await parseJsonBody(request, addDrawingSelectionItemBodySchema);
    const { idempotencyKey } = parseIdempotencyHeaders(request);
    return await idempotentCommandResponse({
      actorId: guard.actor.id,
      operation: "projects.drawing-selection-item.add",
      idempotencyKey,
      request: { path, body },
      execute: async (transaction) => ({
        status: 201,
        body: await addDrawingSelectionItem(
          {
            ...path,
            ...body,
            actorId: guard.actor.id,
...
export const POST = withRequestObservability(
  { module: "drawings", operation: "add-drawing-selection-item" },
  addSelectionItem
);
