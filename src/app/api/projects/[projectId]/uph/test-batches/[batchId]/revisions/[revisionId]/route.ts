...
import { patchUphTestBatchRevision } from "@/modules/uph/application/uph-test-batch-service";
import { uphApiErrorResponse } from "@/modules/uph/application/uph-api-errors";
import { patchUphTestBatchRevisionBodySchema } from "@/modules/uph/contracts/uph-test-batch-http";
...
async function patch(request: Request, context: Context) {
  try {
    ...
    const body = await parseJsonBody(request, patchUphTestBatchRevisionBodySchema);
    ...
    return await idempotentCommandResponse({
      ...
      execute: async (transaction) => ({
        status: 200,
        body: await patchUphTestBatchRevision(serviceInput, transaction)
      })
    });
  } catch (error) {
...
export const PATCH = withRequestObservability(
  { module: "uph", operation: "patch-test-batch-revision" },
  patch
);
