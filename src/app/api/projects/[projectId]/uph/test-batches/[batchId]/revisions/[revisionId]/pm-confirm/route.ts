...
import { confirmUphTestBatch } from "@/modules/uph/application/uph-test-batch-service";
import { uphApiErrorResponse } from "@/modules/uph/application/uph-api-errors";
import { confirmUphTestBatchBodySchema } from "@/modules/uph/contracts/uph-test-batch-http";
...
async function confirm(request: Request, context: Context) {
  try {
    ...
    const body = await parseJsonBody(request, confirmUphTestBatchBodySchema);
    ...
    return await idempotentCommandResponse({
      ...
      execute: async (transaction) => ({
        status: 200,
        body: await confirmUphTestBatch(serviceInput, transaction)
      })
    });
  } catch (error) {
...
export const POST = withRequestObservability(
  { module: "uph", operation: "confirm-test-batch" },
  confirm
);
