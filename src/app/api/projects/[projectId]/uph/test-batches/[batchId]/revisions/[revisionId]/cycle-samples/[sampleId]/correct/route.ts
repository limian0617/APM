...
import { correctUphCycleSample } from "@/modules/uph/application/uph-test-batch-service";
import { uphApiErrorResponse } from "@/modules/uph/application/uph-api-errors";
import { correctUphCycleSampleBodySchema } from "@/modules/uph/contracts/uph-test-batch-http";
...
type Context = {
  params: Promise<{ projectId: string; batchId: string; revisionId: string; sampleId: string }>;
};

async function correct(request: Request, context: Context) {
  try {
    ...
    const body = await parseJsonBody(request, correctUphCycleSampleBodySchema);
    ...
    return await idempotentCommandResponse({
      ...
      execute: async (transaction) => ({
        status: 201,
        body: await correctUphCycleSample(serviceInput, transaction)
      })
    });
  } catch (error) {
...
export const POST = withRequestObservability(
  { module: "uph", operation: "correct-cycle-sample" },
  correct
);
