...
import { appendUphCycleSample } from "@/modules/uph/application/uph-test-batch-service";
import { uphApiErrorResponse } from "@/modules/uph/application/uph-api-errors";
import { appendUphCycleSampleBodySchema } from "@/modules/uph/contracts/uph-test-batch-http";
...
async function append(request: Request, context: Context) {
  try {
    ...
    const body = await parseJsonBody(request, appendUphCycleSampleBodySchema);
    ...
    return await idempotentCommandResponse({
      ...
      execute: async (transaction) => ({
        status: 201,
        body: await appendUphCycleSample(serviceInput, transaction)
      })
    });
  } catch (error) {
...
export const POST = withRequestObservability(
  { module: "uph", operation: "append-cycle-sample" },
  append
);
