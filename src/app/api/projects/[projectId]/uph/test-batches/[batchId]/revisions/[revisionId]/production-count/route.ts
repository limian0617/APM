...
import { updateUphTestBatchProductionCount } from "@/modules/uph/application/uph-test-batch-service";
import { uphApiErrorResponse } from "@/modules/uph/application/uph-api-errors";
import { updateUphTestBatchProductionCountBodySchema } from "@/modules/uph/contracts/uph-test-batch-http";
...
async function put(request: Request, context: Context) {
  try {
    ...
    const body = await parseJsonBody(request, updateUphTestBatchProductionCountBodySchema);
    ...
    return await idempotentCommandResponse({
      ...
      execute: async (transaction) => ({
        status: 200,
        body: await updateUphTestBatchProductionCount(serviceInput, transaction)
      })
    });
  } catch (error) {
...
export const PUT = withRequestObservability(
  { module: "uph", operation: "update-production-count" },
  put
);
