...
import { replaceUphTestBatchRevision } from "@/modules/uph/application/uph-test-batch-service";
import { uphApiErrorResponse } from "@/modules/uph/application/uph-api-errors";
import { replaceUphTestBatchRevisionBodySchema } from "@/modules/uph/contracts/uph-test-batch-http";
...
async function replace(request: Request, context: Context) {
  try {
    ...
    const body = await parseJsonBody(request, replaceUphTestBatchRevisionBodySchema);
    ...
    return await idempotentCommandResponse({
      ...
      execute: async (transaction) => ({
        status: 201,
        body: await replaceUphTestBatchRevision(serviceInput, transaction)
      })
    });
  } catch (error) {
...
export const POST = withRequestObservability(
  { module: "uph", operation: "replace-test-batch-revision" },
  replace
);
