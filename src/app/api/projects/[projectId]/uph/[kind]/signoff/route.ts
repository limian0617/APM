...
import { signoffUphDefinition } from "@/modules/uph/application/uph-definition-service";
...
async function signoff(request: Request, context: Context) {
  const raw = await context.params;
  const path = parsePath(pathSchema, raw);
  const guard = await authorizeProjectRequest(
    request,
    path.projectId,
    PERMISSIONS.PROJECT_UPH_COMMISSIONING_SIGNOFF,
  ...
  try {
    ...
    return await idempotentCommandResponse({
      actorId: guard.actor.id,
      operation: "projects.uph.definition.signoff",
      idempotencyKey,
      request: { path, body },
      execute: async (transaction) => ({
        status: 200,
        body: await signoffUphDefinition(
          {
            ...path,
            ...body,
            actorId: guard.actor.id,
...
export const POST = withRequestObservability(
  { module: "uph", operation: "signoff-uph-definition" },
  signoff
);
