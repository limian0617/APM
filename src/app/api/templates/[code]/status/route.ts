...
import { setProjectTemplateEnabled } from "@/modules/configuration/application/template-service";
...
async function setStatus(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return await idempotentCommandResponse({
      ...
      operation: "configuration.template.status",
      idempotencyKey,
      request: { path, body },
      execute: async (transaction) => ({
        status: 200,
        body: await setProjectTemplateEnabled(
          {
            ...body,
            code: path.code,
            actorId: guard.actor.id,
...
export const PUT = withRequestObservability(
  { module: "configuration-templates", operation: "set-template-status" },
  setStatus
);
