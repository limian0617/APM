...
async function listTags(request: Request) {
  ...
  try {
    ...
    return Response.json(await listProcessTags({ activeOnly: query.activeOnly }));
  } catch (error) {
    const response =
...
async function createTag(request: Request) {
  ...
  try {
    ...
    return await idempotentCommandResponse({
      ...
      operation: "configuration.process-tag.create",
      idempotencyKey,
      request: { body },
      execute: async (transaction) => ({
        status: 201,
        body: await createProcessTag(
          {
            ...body,
            actorId: guard.actor.id,
...
export const GET = withRequestObservability(
  { module: "drawings", operation: "list-process-tags" },
  listTags
);
export const POST = withRequestObservability(
  { module: "drawings", operation: "create-process-tag" },
  createTag
);
