...
describe("APM-033 Gate conditional release persistence contract", () => {
  ...
  it("keeps the APM-034 prerequisite migration in APM-050 CI upgrade coverage", () => {
    const workflow = readFileSync(resolve(process.cwd(), ".github/workflows/ci.yml"), "utf8");

    expect(workflow).toContain("Validate APM-050 to APM-051 upgrade migration");
    expect(workflow).toContain("cp -R prisma/migrations/20260804060000_apm_034_alert_governance");
    expect(workflow).toContain(
      "cp -R prisma/migrations/20260805030000_apm_050_controlled_documents"
    );
  });
});
