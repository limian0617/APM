...
RUN_DATABASE_INTEGRATION=1 APM104_RESTRICTED_NO_EXTENSION=1 APM104_FIXTURE_SETUP_DATABASE_URL="$owner_url" PGPASSWORD="$noext_password" DATABASE_URL="$noext_url" npm run test -- src/modules/knowledge/infrastructure/knowledge-search-capability.integration.test.ts
...
APM104_FIXTURE_SETUP_DATABASE_URL= RUN_DATABASE_INTEGRATION=1 APM104_NORMAL_TRIGRAM=1 PGPASSWORD="$owner_password" DATABASE_URL="postgresql://${owner_user}:${url_password}@${host}:${port}/${normal_db}" npm run test -- src/modules/knowledge/infrastructure/knowledge-search-capability.integration.test.ts
